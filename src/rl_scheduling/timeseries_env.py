...
from .loaders import _server_from_row
from .models import Allocation, Job, Server
from .workload_scenarios import generate_scenario_jobs


class CidtTimeseriesDqnEnv(gym.Env):
    metadata = {"render_modes": []}

    def __init__(
        self,
        snapshots: list[tuple[str, list[Server]]],
        *,
        scenario: str = "normal",
        job_count: int = 48,
        ...
        super().__init__()
        if not snapshots:
            raise ValueError("At least one time-series snapshot is required.")
    ...
    def reset(self, *, seed: int | None = None, options: dict[str, Any] | None = None) -> tuple[np.ndarray, dict[str, Any]]:
        ...
        if seed is not None:
            self._rng.seed(seed)
        options = options or {}
        snapshot_index = options.get("snapshot_index")
        if snapshot_index is None:
            snapshot_index = self._rng.randrange(len(self.snapshots))
        self.timestamp, self.servers = self.snapshots[int(snapshot_index) % len(self.snapshots)]
        self.pool = [MutableServer(server) for server in self.servers]
        job_seed = int(options.get("job_seed", self._rng.randrange(1_000_000_000)))
        self.jobs = sorted(
            generate_scenario_jobs(self.scenario, self.job_count, self.servers, seed=job_seed),
            key=lambda item: (-item.priority, item.deadline_minutes, item.job_id),
        )
        self.job_index = 0
    ...
    def _observation(self) -> np.ndarray:
        ...
        down_ratio = 1.0 - (len(active) / max(len(self.pool), 1))
        progress = self.job_index / max(len(self.jobs), 1)
    ...
    def _placement_reward(self, server: MutableServer, job: Job) -> float:
        ...
        capacity_bonus = min(server.mem_available_gb / max(job.memory_gb, 1.0), 5.0) * 0.15
        imbalance_penalty = abs(cpu_util - 0.70) * 2.0
        return 10.0 + job.priority + utilization_bonus + deadline_bonus + rack_bonus + capacity_bonus - imbalance_penalty
...
def load_timeseries_snapshots(path: str | Path, *, max_snapshots: int = 0) -> list[tuple[str, list[Server]]]:
    snapshots = list(_iter_snapshots(Path(path)))
    return _even_sample(snapshots, max_snapshots) if max_snapshots else snapshots


def _iter_snapshots(path: Path) -> Iterable[tuple[str, list[Server]]]:
...
def _even_sample(snapshots: list[tuple[str, list[Server]]], count: int) -> list[tuple[str, list[Server]]]:
